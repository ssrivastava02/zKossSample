...
public class LandingPage extends SelectorComposer<Window> {
	...
	@Wire 
	private Radiogroup fruitsRadiogroup;
	...
	@Wire
	private Grid displayUser;

	@Listen("onClick = button#printButton")
	public void printSelectedFruit() {
		Radio selectedRadio = fruitsRadiogroup.getSelectedItem();

		if(selectedRadio != null) {
			//Extraction of values
			String selectedFruit = selectedRadio.getLabel();
			String userName = inputUserName.getValue();

			//Conversion to label for later appending
			Label userLable = new Label(userName);
			Label fruitLable = new Label(selectedFruit);
			...
			Messagebox.show("Please select a fruit !" , "Warning", Messagebox.OK, Messagebox.EXCLAMATION);  
		}
	}
	@Listen("onMouseOver = button#printButton")
	public void changeColor() {
		printButton.setStyle("background-color: #D04848; color: #ffffff; height=30%; width=30%; style= white-space: normal; margin-left: 30%;");
	}
	@Listen("onMouseOut = button#printButton")
